...
(function ($) {
  ...
  $.fn.Page = function (options) {
    ...
    return this.each(function () {
      ...
      var dataHtml = '';
       if(opts.hasGopage){
          dataHtml += '<div class="goto fr"> <div  class="fl yepage">共 '+l+'页</div> <input type=" text"  class="goinput fl"><div class="gopage fl">' + opts.gopage + '</div></div>';
      }
      if(opts.hasNext){
      ...
      //goto
     	 obj.on('click', '.gopage', function(){
      	 ...
      	 var pagecur =parseInt($('.goinput').val());
        //当前页
          if(pagecur != "" && pagecur <= l){
          	 var nums = odevity(n);
          	opts.callBack(pagecur);
          var	pagge = pagecur-1
          	 if(l>n){
		          if(pagecur > l-nums/2&&pagecur < l+1){
		            //最后几项
		            fpageNext((n-1)-(l-pagecur));
		          }else if(pagecur > 0&&pagecur < nums/2){
		            //最前几项
		            fpagePrv(pagecur-1);
		          }else{
		            fpageShow(pagecur);
		          ...
		          $('.' + active).removeClass(active);
		       	  $('#page li').eq(pagge).find('a').addClass(active);
		        }
